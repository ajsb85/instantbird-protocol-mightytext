...
XPCOMUtils.defineLazyGetter(this, "_", function()
  l10nHelper("chrome://prpl-mightytext/locale/mightytext.properties")
);
...
Conversation.prototype = {
  ...
  sendMsg: function (aMsg) {
    if (this._disconnected) {
      this.writeMessage("mightytext", "This message could not be sent because the conversation is no longer active: " + aMsg, {system: true, error: true});
      return;
    }
...
Account.prototype = {
  connect: function() {
    ...
    setTimeout((function() {
      ...
      this._conv.writeMessage("mightytext", "You are now talking to /dev/null", {system: true});
    }).bind(this), 0);
  },
  _conv: null,
  disconnect: function(aSilent) {
    ...
    if (!aSilent)
      this._conv.writeMessage("mightytext", "You have disconnected.", {system: true});
    if (this._conv) {
...
jsTest2Protocol.prototype = {
  get normalizedName() "mightytext",
  get name() "MightyText",
  get iconBaseURI() "chrome://prpl-mightytext/skin/",
  options: {
  ...
  classID: Components.ID("{9af4bb00-a94e-11e3-a5e2-0800200c9a66}"),
};
